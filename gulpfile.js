...
exports.dev = gulp.series(styles, server, watcher);
...
const copyFonts = () => {
  return gulp.src('source/fonts/*.{woff,woff2,ttf}').pipe(gulp.dest('build/fonts'));
};
...
const js = () => {
  return gulp.src('source/js/*.js').pipe(jsMin()).pipe(rename('script.min.js')).pipe(gulp.dest('build/js'));
};
exports.js = js;
...
const images = () => {
  return gulp
    ...
    .pipe(imageMin([imageMin.optipng({ optimizationLevel: 3 }), imageMin.mozjpeg({ progressive: false })]))
...
const copyPHP = () => {
  return gulp.src('source/js/*.php').pipe(gulp.dest('build/js'));
};
